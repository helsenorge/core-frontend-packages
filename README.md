...
The code in this repository is maintained in a private Git repository also containing code we have not yet open sourced. We use [Gitexporter](https://github.com/open-condo-software/gitexporter) to create the open source repository with only the NPM packages used in our other open source projects ([Refero](https://github.com/helsenorge/refero) and [Structor](https://github.com/helsenorge/structor)). 

The private Git repository is a monorepo containing other NPM packages, webpages and micro web for helsenorge.no and use NPM workspaces and Lerna. Since this will not work without all the code from the monorepo each package has to be built and tested separately for now.
