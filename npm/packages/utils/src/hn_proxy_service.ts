...
type RequestParamType = Record<
  string,
  string | number | boolean | Array<RequestParamArrayType>
>;

// ProxyOperationResponse returneres bare ved feil fra proxy-tjenester
export interface ProxyOperationResponse extends Response {
  Code: string;
    ...
    Object.keys(params)
      .map((key: string) => {
        return Array.isArray(params[key])
          ? (params[key] as Array<RequestParamArrayType>)
              .map((x: RequestParamArrayType) => `${key}=${x}`)
              .join("&")
          : `${key}=${params[key]}`;
      })
      .join("&")
...
const checkStatus = <T extends ProxyOperationResponse>(
  ...
  if (contentType && contentType.indexOf("application/json") !== -1) {
    ...
    return response.json().then((err: ProxyOperationResponse) => {
      if (err.Code === "SEC-110000") {
...
const baseCrud = <T extends ProxyOperationResponse, R>(
  method: string,
  url: string,
  proxyName: string,
  ...
  const queryString =
    params && Object.keys(params).length > 0 ? createQueryString(params) : "";
  ...
  return fetch(baseUrl + url + queryString, {
    ...
    .then((response: T) => checkStatus<T>(response))
    .catch(err => {
      trackError("level1");
...
export const get = <T extends ProxyOperationResponse>(
  url: string,
  proxyName: string,
  params?: RequestParamType
): Promise<T | null> => {
...
export const post = <T extends ProxyOperationResponse, R>(
  url: string,
  proxyName: string,
  data: R,
  params?: RequestParamType
...
export const put = <T extends ProxyOperationResponse, R>(
  url: string,
  proxyName: string,
  data?: R,
  params?: RequestParamType
...
export const remove = <T extends ProxyOperationResponse, R>(
  url: string,
  proxyName: string,
  data?: R,
  params?: RequestParamType
...
export const link = (
  ...
  return (
    getProxyEnvironmentPath(proxyName) +
    url +
    createQueryString({ ...getDefaultRequestParams(), ...params })
  );
};
