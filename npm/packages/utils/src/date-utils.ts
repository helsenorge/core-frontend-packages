...
export function addDifference(
  target: ISO8601,
  originalStart: ISO8601,
  originalStop: ISO8601
): ISO8601 {
  const diff =
    new Date(originalStop).getTime() - new Date(originalStart).getTime();
...
export function getFormattedDateString(
  dateString: string,
  resources: ResourcesWithMonthNames
): string | null {
