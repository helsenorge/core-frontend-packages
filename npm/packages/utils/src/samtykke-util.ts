...
export function hasDigitaleHelsetjenesteSamtykke(samtykker: Samtykke[]) {
  if (samtykker) {
    return samtykker.some(
      s =>
        s.PersonvernInnstillingDefinisjonGuid.toUpperCase() ===
          PersonvernInnstillingDefinisjonGuids.DigitalHelsetjeneste && s.ErAktiv
    );
  }
}
export function hasPasientreiserSamtykke(samtykker: Samtykke[]) {
  if (samtykker) {
    return samtykker.some(
      s =>
        s.PersonvernInnstillingDefinisjonGuid.toUpperCase() ===
          PersonvernInnstillingDefinisjonGuids.Pasientreiser && s.ErAktiv
    );
  }
}
...
export function getSamtykker(): Array<Samtykke> {
  if (
    window.HN.Commands.__GetTjenesterMedTilgang__ !== undefined &&
    window.HN.Commands.__GetTjenesterMedTilgang__ !== null
  ) {
...
export function harSamtykket(
  guid: PersonvernInnstillingDefinisjonGuids
): boolean {
  ...
  return (
    samtykker.findIndex(samtykke => {
      return (
        samtykke.PersonvernInnstillingDefinisjonGuid.toUpperCase() ===
          guid.toUpperCase() && samtykke.ErAktiv
      );
    }) > -1
  );
}
