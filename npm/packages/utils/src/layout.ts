...
export default {
  isOneToTwoColumn: function(): boolean {
    return window.matchMedia(
      `screen and (max-width: ${LayoutConstants.ONE_TO_TWO_COLUMN})`
    ).matches;
  },
  isTwoToThreeColumn: function(): boolean {
    return window.matchMedia(
      `screen and (max-width: ${LayoutConstants.TWO_TO_THREE_COLUMN})`
    ).matches;
  },
  isThreeTwoFourColumn: function(): boolean {
    return window.matchMedia(
      `screen and (max-width: ${LayoutConstants.THREE_TO_FOUR_COLUMN})`
    ).matches;
  }
};
